...
import type { Booking, BookingTab } from "@/types/domain/booking";
 ...
 *
 * [티켓 조회 정책]
 *  - 티켓 보기 클릭 → 티켓 상세 페이지 이동
 */
export function BookingCard({ booking, tab }: BookingCardProps) {
  const navigate = useNavigate();

  const concertDate = new Date(booking.concertDate);
  const bookedAtDate = new Date(booking.bookedAt);

  // ─ 환불 가능 여부 계산 (D-7 기준) ─
  const now = new Date();
  const msUntilConcert = concertDate.getTime() - now.getTime();
  const daysUntilConcert = msUntilConcert / (1000 * 60 * 60 * 24);
  const isRefundable = daysUntilConcert >= 7 && booking.status === "CONFIRMED";
  ...
  const handleViewTicket = () => {
    navigate(`/bookings/${booking.bookingId}/ticket`);
  };

  const handleRefund = () => {
    // TODO: Sprint 9 환불 모달 연동 (Feat #27)
    if (window.confirm("정말 환불을 신청하시겠습니까?")) {
      // refundMutation.mutate(booking.bookingId);
    }
  };
    ...
    <article className="bg-white border border-gray-200 rounded-lg p-6">
      ...
      <div className="space-y-1 text-sm text-gray-600 mb-4">
        <div className="flex items-center gap-1.5">
          ...
          <span>{formatDateTime(concertDate)}</span>
        </div>
        <div className="flex items-center gap-1.5">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1">
            {booking.seats.map((seat) => (
              <span
                key={seat}
                className="text-xs px-2 py-0.5 rounded-full bg-primary text-white font-medium"
              >
                {seat}
              </span>
            ))}
