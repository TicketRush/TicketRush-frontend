...
export default function DatePartsInput({ id, value, onChange }: Props) {
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      <label htmlFor={`${id}-year`} className="space-y-1 text-sm">
        <span>연도</span>
        <input id={`${id}-year`} className={inputClass} type="text" inputMode="numeric" maxLength={4}
      ...
      <label htmlFor={`${id}-month`} className="space-y-1 text-sm">
        <span>월</span>
        <select id={`${id}-month`} className={inputClass} value={value.month} disabled={!validYear}
      ...
      <label htmlFor={`${id}-day`} className="space-y-1 text-sm">
        <span>일</span>
        <select id={`${id}-day`} className={inputClass} value={value.day} disabled={!days}
